...
class Command(BaseCommand):
    help = 'Copy a user from main database to tenant database (for testing)'

    def add_arguments(self, parser):
        parser.add_argument('username', type=str, help='Username to copy')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully copied user {username} to tenant {tenant_name}")
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error copying user: {str(e)}")
            )
            import traceback
